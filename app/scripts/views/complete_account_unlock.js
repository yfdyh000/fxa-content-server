...
function (FormView, BaseView, CompleteAccountUnlockTemplate, AuthErrors,
  ...
  function doesLinkValidate(uid, code, isLinkDamaged) {
    return Validate.isUidValid(uid) &&
           Validate.isCodeValid(code) &&
           ! isLinkDamaged;
  }
  ...
  var CompleteAccountUnlockView = FormView.extend({
    ...
    beforeRender: function () {
      ...
      if (! doesLinkValidate(this.uid, this.code, this._isLinkDamaged)) {
    ...
    context: function () {
      var isLinkValid = doesLinkValidate(
              this.uid, this.code, this._isLinkDamaged);
      var isLinkExpired = this._isLinkExpired;

      return {
        // If the link is invalid, print a special error message.
        isLinkDamaged: ! isLinkValid,
